...
router.get("/advisor/clients/:id/kyc", requireAuth, async (req, res): Promise<void> => {
  const rawId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  res.json(await db.select().from(kycDocumentsTable).where(eq(kycDocumentsTable.userId, rawId)));
});

router.post("/advisor/clients/:id/kyc/:docId/verify", requireAuth, async (req, res): Promise<void> => {
  ...
  }).where(eq(kycDocumentsTable.id, rawDocId)).returning();
